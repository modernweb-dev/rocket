...
describe('markdown conversion', async () => {
  const md03mdHtml = [
    '<server-code>',
    "let foo = 'bar';",
  ...
  it('1b. escapes back slashes in markdown', async () => {
    expect(
      mdToMdInJs(
        [
          //
  ...
  it('2. render markdown to html while wrapping server code and keeping it as is', async () => {
    expect(
  ...
  it('3. create the final template to be used for rendering', async () => {
  ...
  it('3b. will return the template directly if there is only one', async () => {
    expect(
      mdHtmlToJsTemplate(
        [
          '<server-code>',
